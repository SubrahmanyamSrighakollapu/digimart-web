  ...
  const renderContent = (content) => {
    return content.map((item, index) => {
      switch (item.type) {
        ...
        case 'points':
          return (
            <ul key={index} className="mb-3" style={{ listStyleType: 'disc', paddingLeft: '0.5rem' }}>
          ...
          return (
            <ul key={index} className="mb-3" style={{ listStyleType: 'disc', paddingLeft: '2rem' }}>
              {item.items.map((point, idx) => {
                ...
                return (
                  <li key={idx} className="terms-content-text mb-2" style={{ paddingLeft: '0.5rem' }}>
                    <strong className="text-dark">{boldText}</strong> {normalText}
                  </li>
                );
              })}
            </ul>
        ...
        case 'subtitle':
          return (
            <ul key={index} className="mb-3" style={{ listStyleType: 'disc', paddingLeft: '0.5rem' }}>
              {item.items.map((point, idx) => (
                <li key={idx} className="terms-content-text mb-2" style={{ paddingLeft: '0.5rem' }}>
                  <strong className="text-dark">{point}</strong>
                </li>
              ))}
            </ul>
          );
        ...
        <div className="column">
          <div className="col-md-6 mb-2 mb-md-0">
            <p className="green mb-0">
              Effective Date: {banner.lastUpdated}
            </p>
          </div>
        </div>
      </div>
